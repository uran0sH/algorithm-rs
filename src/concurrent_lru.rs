    ...
    T: std::fmt::Debug,
{
    ...
    pub fn insert_front(&self, val: T) {
        // println!("{:?} insert_front", std::thread::current().id());
    ...
    pub fn insert_front_raw(&self, mut node: NonNull<Node<T>>) {
        // println!("{:?} insert_front_raw", std::thread::current().id());
    ...
    pub fn remove(&self, mut node: NonNull<Node<T>>) -> T {
        // println!("{:?} remove", std::thread::current().id());
    ...
    pub fn reinsert_front(&self, mut node: NonNull<Node<T>>) {
        // println!("{:?} reinsert front", std::thread::current().id());
        {
    ...
    pub fn remove_tail(&self) -> Option<T> {
        // println!("{:?} remove_tail", std::thread::current().id());
    ...
    T: std::fmt::Debug,
{
    ...
    pub fn insert(&self, key: &[u8], value: T) -> Option<T> {
        ...
        println!(
            ...
            usize::from_le_bytes(key.try_into().unwrap())
        );
